...
public class CreditNoteDao {
	...
	private final static Logger LOGGER = LogManager.getLogger(CreditNoteDao.class);
	...
	@Transactional
	public CreditNoteDO getInvoiceDetails(String invId) {
		LOGGER.info("DebitNoteDao :: getInvoiceDetails :: Start ");
		CreditNoteDO invDO = null;
		try {
			...
			CriteriaQuery<CreditNoteDO> query = builder.createQuery(CreditNoteDO.class);

			Root<CreditNoteDO> root = query.from(CreditNoteDO.class);
    ...
    public String saveInvoice(CreditNoteDO invoiceDO) {
		LOGGER.info("DebitNoteDao::saveInvoice::start");

		try {
			CreditNoteDO tmp = invoiceDO;
...
//			tx.commit();
    ...
    public List<CreditNoteDO> getInvoiceList(String financialYear) {
		LOGGER.info("DebitNoteDao :: getInvoiceList :: Start ");
		List<CreditNoteDO> invoiceList = new ArrayList<CreditNoteDO>();
		try {
    ...
    public List<CreditNoteDO> getInvoiceList(Map<String, String> data) {
		LOGGER.info("DebitNoteDao :: getInvoiceList :: Start ");
		List<CreditNoteDO> invoiceList = new ArrayList<CreditNoteDO>();
		try {
...
//			Predicate categoryPredicate=builder.equal(root.get(null), statusPredicate)
    ...
    public List<CreditNoteDO> getInvoiceListByMonth(String month,String financialYear) {
	...
	public boolean DeleteInvoice(String invId) {
		...
		try {
			...
			Query query = session.createNativeQuery("delete from credit_note where credit_note_Id ='" + invId + "'");
    ...
    public boolean cloneInvoice(String invNo) {
		...
		boolean flag=false;
		CreditNoteDO invDO= new CreditNoteDO();
		List<CreditNoteDO> invtemp = new ArrayList<>();
		Integer gInvNo = 0;
		String fInvNo="";
		try {
...
//			gInvNo = invtemp.size();
			...
			List<CreditNoteProductDO> productdo = new ArrayList<CreditNoteProductDO>();
			for(CreditNoteProductDO temp : invDO.getInvoiceProductDO()) {
				CreditNoteProductDO invprod = new CreditNoteProductDO();
				invprod = temp;
    ...
    public boolean saveCloneInv(CreditNoteDO invdo) {
...
public boolean cancelInvoiceById(Integer invoiceId) {
	...
	try {
		...
		CreditNoteDO invoiceDO=getInvoiceDetails(invoiceId.toString());
...
public List<CreditNoteDO> getInvoiceByFinancialYear(String financialYear) {
	LOGGER.info("DebitNoteDao::getInvoiceByFinancialYear()::Start");
	List<CreditNoteDO> invoiceDO=null;
	...
	try {
		...
		CriteriaQuery<CreditNoteDO> query=criteriaBuilder.createQuery(CreditNoteDO.class);
