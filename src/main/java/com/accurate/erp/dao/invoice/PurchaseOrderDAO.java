...
import com.accurate.erp.model.invoice.PurchaseOrderDO;
import com.accurate.erp.model.invoice.PurchaseOrderProductDO;
import com.accurate.erp.model.modelmaster.DocumentSeqMasterDO;

@Repository
public class PurchaseOrderDAO {
	
	private final static Logger LOGGER = LogManager.getLogger(PurchaseOrderDAO.class);
	...
	@Transactional
	public PurchaseOrderDO getInvoiceDetails(String invId) {
		LOGGER.info("SupplierQuotationDAO :: getInvoiceDetails :: Start ");
		PurchaseOrderDO invDO = null;
		try {
			...
			CriteriaQuery<PurchaseOrderDO> query = builder.createQuery(PurchaseOrderDO.class);
    ...
    public String saveInvoice(PurchaseOrderDO invoiceDO) {
		LOGGER.info("SupplierQuotationDAO::saveInvoice::start");

		try {
			PurchaseOrderDO tmp = invoiceDO;
			PurchaseOrderDO invDO = null;
...
//			tx.commit();
    ...
    public List<PurchaseOrderDO> getInvoiceList() {
		LOGGER.info("SupplierQuotationDAO :: getInvoiceList :: Start ");
		List<PurchaseOrderDO> invoiceList = new ArrayList<PurchaseOrderDO>();
		try {
    ...
    public List<PurchaseOrderDO> getInvoiceList(Map<String, String> data) {
		LOGGER.info("SupplierQuotationDAO :: getInvoiceList :: Start ");
		List<PurchaseOrderDO> invoiceList = new ArrayList<PurchaseOrderDO>();
		try {
...
//			Predicate categoryPredicate=builder.equal(root.get(null), statusPredicate)
    ...
    public String getInvNo(){
		...
		PurchaseOrderDO invoiceDO = null;
		String tempInv = "";
		List<String> temp;
		try {
			...
			Predicate predicate=builder.equal(root.get("documentName"), "PurchaseOrder");
			...
			Root<PurchaseOrderDO> root2=query1.from(PurchaseOrderDO.class);
    ...
    public List<PurchaseOrderDO> getInvoiceListByMonth(String month) {
	...
	public boolean DeleteInvoice(String invId) {
		...
		try {
			...
			Query query = session.createNativeQuery("delete from Purchase_Order where PO_Id ='" + invId + "'");
    ...
    public boolean cloneInvoice(String invNo) {
		...
		boolean flag=false;
		PurchaseOrderDO invDO= new PurchaseOrderDO();
		List<PurchaseOrderDO> invtemp = new ArrayList<>();
		Integer gInvNo = 0;
		String fInvNo="";
		try {
			...
			List<PurchaseOrderProductDO> productdo = new ArrayList<PurchaseOrderProductDO>();
			for(PurchaseOrderProductDO temp : invDO.getInvoiceProductDO()) {
				PurchaseOrderProductDO invprod = new PurchaseOrderProductDO();
				invprod = temp;
    ...
    public boolean saveCloneInv(PurchaseOrderDO invdo) {
...
public boolean cancelInvoiceById(Integer invoiceId) {
	...
	try {
		...
		PurchaseOrderDO invoiceDO=getInvoiceDetails(invoiceId.toString());
...
public List<PurchaseOrderDO> getInvoiceByFinancialYear(String financialYear) {
	LOGGER.info("SupplierQuotationDAO::getInvoiceByFinancialYear()::Start");
	List<PurchaseOrderDO> invoiceDO=null;
	...
	try {
		...
		CriteriaQuery<PurchaseOrderDO> query=criteriaBuilder.createQuery(PurchaseOrderDO.class);
